...
import { PreFinancingEntity } from './pre-financing.entity';
import { PreFinancingModel } from './pre-financing.model';
...
export class PreFinancingService {
  private repository: Repository<PreFinancingEntity>;
  private relationship = ['campagne', 'exportateur', 'compte', 'sousCompte'];
  ...
  constructor(
    ...
    if (connection == undefined) {
      ...
      this.repository = connection.getRepository(PreFinancingEntity);
    }
  }

  async save(input: IRmqInput<PreFinancingModel>): Promise<any> {
    ...
    if (this.connectionOk) {
      ...
      if (userExist) {
        try {
          const movement = input.value.mouvement;
  ...
  async update(input: IRmqInput<PreFinancingModel>): Promise<any[]> {
  ...
  // async delete(input: IRmqInput<PreFinancingModel>): Promise<any[]> {
  //   let exception: any, result: any;
  //   if (this.connectionOk) {
  //     const userExist = await this.verifUser.checkExistingUser(
  //       this.connection,
  ...
  async softDelete(input: IRmqInput<PreFinancingModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async restore(input: IRmqInput<PreFinancingModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
        ...
        try {
          ...
          if (isDefined(found)) {
            for (const item of found) {
              ...
              const subAccount = isDefined(item.sousCompte)
                ? cleanUp(item.sousCompte, clean)
                : null;
        ...
        try {
          ...
          if (isDefined(found)) {
            ...
            const exportateur = isDefined(found.exportateur)
              ...
              : null;
            const mainResult = await this.getMainObject(found.compte.banque_id);
            // remove movement; account info; sub account info
            ...
            const subAccount = isDefined(found.sousCompte)
              ? cleanUp(found.sousCompte, clean)
              : null;
  ...
  async getMainObject(bank: UUIDVersion) {
    const object = {};
