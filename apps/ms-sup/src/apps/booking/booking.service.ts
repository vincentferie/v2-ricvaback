...
import { FileBookingModel } from './file/file-booking.model';

@Injectable()
@TenantDecorateurService()
export class BookingService {
  private repository: Repository<BookingEntity>;
  private subRepository: Repository<FileBookingEntity>;
  private connectionOk = true;
  private relationship = ['file'];

  constructor(
    @Inject(TENANT_CONNECTION) private readonly connection: Connection,
    private readonly verifUser: VerifUser, // private readonly mainComponent: MainComponentService,
  ) {
    if (connection == undefined) {
      this.connectionOk = false;
    } else {
      this.repository = connection.getRepository(BookingEntity);
      this.subRepository = connection.getRepository(FileBookingEntity);
    }
  }

  async save(input: IRmqInput<BookingModel>): Promise<any> {
  ...
  async update(input: IRmqInput<BookingModel>): Promise<any[]> {
  ...
  async close(input: IRmqInput<BookingModel>): Promise<any[]> {
    let exception: any, result: any;
  ...
  async delete(input: IRmqInput<BookingModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
      ...
      if (userExist) {
        ...
        try {
          ...
          if (isDefined(result)) {
            ...
            if (isDefined(query)) {
              await this.subRepository.delete({ booking_id: input.param });
  ...
  async softDelete(input: IRmqInput<BookingModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async restore(input: IRmqInput<BookingModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
      ...
      if (userExist) {
        try {
          const [found, total] = await this.repository.findAndCount({
            ...
            where: [
              {
                numero_reel: ILike(`%${input.value.query}%`),
              },
              {
                numero_change: ILike(`%${input.value.query}%`),
              },
            ],
            order: { created_date: input.value.order ?? 'DESC' },
            skip: +input.value.offset,
  ...
  async findCompletion(input: IRmqInput<QueryParam>): Promise<any[]> {
    ...
    if (this.connectionOk) {
      ...
      if (userExist) {
        try {
          const [found, total] = await this.repository.findAndCount({
            select: ['id', 'numero_reel', 'numero_change'],
            withDeleted: false,
            where: [
              {
                numero_reel: Raw(
