...
import { CampagneTrancheEntity } from './tranche.entity';
import { CampagneTrancheModel } from './tranche.model';

@Injectable()
@TenantDecorateurService()
export class CampagneTrancheService {
  private repository: Repository<CampagneTrancheEntity>;
  private relationship = ['campagne'];
  ...
  constructor(
    ...
    if (connection == undefined) {
      ...
      this.repository = connection.getRepository(CampagneTrancheEntity);
    }
  }

  async save(input: IRmqInput<CampagneTrancheModel>): Promise<any> {
  ...
  async update(input: IRmqInput<CampagneTrancheModel>): Promise<any[]> {
  ...
  async delete(input: IRmqInput<CampagneTrancheModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async softDelete(input: IRmqInput<CampagneTrancheModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async restore(input: IRmqInput<CampagneTrancheModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
        ...
        try {
          ...
          if (isDefined(found)) {
            for (const item of found) {
              ...
              const campagne = cleanUp(item.campagne, clean);
              result.push({
                ...finalItem,
                campagne: campagne,
              });
      ...
      if (userExist) {
        try {
          ...
          if (isDefined(found)) {
            ...
            const campagne = cleanUp(found.campagne, clean);
            result = {
              ...finalItem,
              campagne: campagne,
            };
