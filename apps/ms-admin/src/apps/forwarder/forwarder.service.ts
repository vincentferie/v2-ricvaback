...
import { ForwarderModel } from './forwarder.model';

@Injectable()
@TenantDecorateurService()
export class ForwarderService {
  private repository: Repository<ForwarderEntity>;
  ...
  constructor(
    ...
    if (connection == undefined) {
      ...
      this.repository = connection.getRepository(ForwarderEntity);
    }
  }

  async save(input: IRmqInput<ForwarderModel>): Promise<any> {
  ...
  async update(input: IRmqInput<ForwarderModel>): Promise<any[]> {
  ...
  async delete(input: IRmqInput<ForwarderModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async softDelete(input: IRmqInput<ForwarderModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async restore(input: IRmqInput<ForwarderModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
        ...
        try {
          ...
          if (isDefined(found)) {
            for (const item of found) {
              ...
              // const entrepots = cleanUp(item.entrepots, clean1);
              result.push({
                ...finalItem,
                // entrepots: entrepots,
              });
      ...
      if (userExist) {
        try {
          ...
          if (isDefined(found)) {
            ...
            // const entrepots = cleanUp(found.entrepots, clean1);
            result = {
              ...finalItem,
              // entrepots: entrepots,
            };
