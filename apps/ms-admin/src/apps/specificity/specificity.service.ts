...
import { SpecificityModel } from './specificity.model';

@Injectable()
@TenantDecorateurService()
export class SpecificityService {
  private repository: Repository<SpecificityEntity>;
  ...
  constructor(
    ...
    if (connection == undefined) {
      ...
      this.repository = connection.getRepository(SpecificityEntity);
    }
  }

  async save(input: IRmqInput<SpecificityModel>): Promise<any> {
  ...
  async update(input: IRmqInput<SpecificityModel>): Promise<any[]> {
  ...
  async delete(input: IRmqInput<SpecificityModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async softDelete(input: IRmqInput<SpecificityModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async restore(input: IRmqInput<SpecificityModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
      ...
      if (userExist) {
        try {
          ...
          if (isDefined(found)) {
            ...
            // const entrepots = cleanUp(found.entrepots, clean2);
            result = {
              ...finalItem,
              // entrepots: entrepots,
            };
