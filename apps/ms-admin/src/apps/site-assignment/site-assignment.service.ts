...
import { SiteAssignmentEntity } from './site-assignment.entity';
import { SiteAssignmentModel } from './site-assignment.model';

@Injectable()
@TenantDecorateurService()
export class SiteAssignmentService {
  private repository: Repository<SiteAssignmentEntity>;
  private connectionOk = true;
  private relationship = ['superviseur', 'site'];
  ...
  constructor(
    ...
    if (connection == undefined) {
      ...
      this.repository = connection.getRepository(SiteAssignmentEntity);
    }
  }

  async save(input: IRmqInput<SiteAssignmentModel>): Promise<any> {
  ...
  async update(input: IRmqInput<SiteAssignmentModel>): Promise<any[]> {
  ...
  async delete(input: IRmqInput<SiteAssignmentModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async softDelete(input: IRmqInput<SiteAssignmentModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async restore(input: IRmqInput<SiteAssignmentModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
          ...
          if (isDefined(found)) {
            for (const item of found) {
              ...
              const superviseur = isDefined(item.superviseur)
                ...
                : null;
              const site = isDefined(item.site)
                ? isDefined(item.site)
                  ? cleanUp(item.site, clean1)
                  : null
                : null;
              result.push({
                ...finalItem,
        ...
        try {
          ...
          if (isDefined(found)) {
            ...
            const superviseur = isDefined(found.superviseur)
              ...
              : null;
            const site = isDefined(found.site)
              ? cleanUp(found.site, clean1)
              : null;
            result = {
              ...finalItem,
              superviseur: superviseur,
