...
export class ExporterService {
  private repository: Repository<ExporterEntity>;
  ...
  constructor(
    ...
    if (connection == undefined) {
      ...
      this.repository = connection.getRepository(ExporterEntity);
    }
  }

  async save(input: IRmqInput<ExporterModel>): Promise<any> {
  ...
  async update(input: IRmqInput<ExporterModel>): Promise<any[]> {
  ...
  async delete(input: IRmqInput<ExporterModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async softDelete(input: IRmqInput<ExporterModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async restore(input: IRmqInput<ExporterModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async findCompletion(input: IRmqInput<QueryParam>): Promise<any[]> {
    ...
    if (this.connectionOk) {
      ...
      if (userExist) {
        try {
          const [found, total] = await this.repository.findAndCount({
            select: ['id', 'raison', 'contact', 'postal', 'email', 'lieu'],
            withDeleted: false,
            where: [
              {
                raison: Raw(
