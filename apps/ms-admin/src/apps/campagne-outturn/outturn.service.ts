...
import { CampagneOutturnModel } from './outturn.model';

@Injectable()
@TenantDecorateurService()
export class CampagneOutturnService {
  private repository: Repository<CampagneOutturnEntity>;
  ...
  constructor(
    ...
    if (connection == undefined) {
      ...
      this.repository = connection.getRepository(CampagneOutturnEntity);
    }
  }

  async save(input: IRmqInput<CampagneOutturnModel>): Promise<any> {
  ...
  async update(input: IRmqInput<CampagneOutturnModel>): Promise<any[]> {
  ...
  async delete(input: IRmqInput<CampagneOutturnModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async softDelete(input: IRmqInput<CampagneOutturnModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async restore(input: IRmqInput<CampagneOutturnModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async findCompletion(input: IRmqInput<QueryParam>): Promise<any[]> {
    ...
    if (this.connectionOk) {
      ...
      if (userExist) {
        try {
          const [found, total] = await this.repository.findAndCount({
            select: ['id', 'flag', 'min_outturn', 'max_outturn'],
            withDeleted: false,
            where: [
              {
                campagne_id: input.value.filter_id,
