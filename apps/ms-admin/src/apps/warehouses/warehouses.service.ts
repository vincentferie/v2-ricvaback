...
export class WarehousesService {
  private repository: Repository<WarehousesEntity>;
  private connectionOk = true;
  private relationship = ['site'];
  ...
  constructor(
    ...
    if (connection == undefined) {
      ...
      this.repository = connection.getRepository(WarehousesEntity);
    }
  }

  async save(input: IRmqInput<WarehousesModel>): Promise<any> {
  ...
  async update(input: IRmqInput<WarehousesModel>): Promise<any[]> {
  ...
  async delete(input: IRmqInput<WarehousesModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async softDelete(input: IRmqInput<WarehousesModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
  ...
  async restore(input: IRmqInput<WarehousesModel>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
      ...
      if (userExist) {
        try {
          ...
          if (isDefined(found)) {
            for (const item of found) {
              const finalItem = cleanUp(item, clean);
              // const entrepots = cleanUp(item.entrepots, clean2);
              result.push({
                ...finalItem,
                // entrepots: entrepots,
              });
  ...
  async findBySite(input: IRmqInput<QueryParam>): Promise<any[]> {
    let exception: any, result: any;
    if (this.connectionOk) {
      const userExist = await this.verifUser.checkExistingUser(
        this.connection,
